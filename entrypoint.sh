...
cd "$BINUTILS_OBJ_DIR" || (echo "could not find dir for binutils"; exit 1;) 
../configure --prefix="$PREFIX" --target=avr --disable-nls && make -j"$(nproc)" && make install

# then gcc with avr as a target
cd "$GCC_BASE_DIR" || (echo "could not find dir for gcc"; exit 1;) 
./contrib/download_prerequisites

mkdir -p "$GCC_OBJ_DIR"
cd "$GCC_OBJ_DIR" || (echo "could not find dir for libc"; exit 1;) 
../configure --prefix="$PREFIX" --target=avr --enable-languages=c,c++ \
--disable-nls --disable-libssp --with-dwarf2 && make -j"$(nproc)" && make install

# and finally libc
mkdir -p "$LIBC_OBJ_DIR"
cd "$LIBC_OBJ_DIR" || (echo "could not find dir for libc"; exit 1;)
